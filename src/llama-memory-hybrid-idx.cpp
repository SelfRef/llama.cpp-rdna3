...
void llama_memory_hybrid_idx::seq_cp(llama_seq_id seq_id_src, llama_seq_id seq_id_dst, llama_pos p0, llama_pos p1) {
    ...
    // are repooled from its own cells on its first ubatch
...
void llama_memory_hybrid_idx::state_read(llama_io_read_i & io, llama_seq_id seq_id, llama_state_seq_flags flags) {
    ...
    try {
        ...
        if ((flags & LLAMA_STATE_SEQ_FLAGS_PARTIAL_ONLY) == 0) {
            ...
            // [TAG_QSA_POOLED_CACHE] a full restore rewrites the indexer cells with
            // arbitrary content, so no pooled row can be trusted; the next ubatch repools
            // the whole range. A PARTIAL_ONLY restore (speculative checkpoint replay)
            // leaves the cells untouched and its rollback arrives through seq_rm, which
            // already clamped the watermark - this branch does not run for it.
            pooled_reset(seq_id);
        }
