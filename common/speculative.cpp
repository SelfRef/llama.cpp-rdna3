    ...
    void draft(common_speculative_draft_params_vec & dparams) override {
        ...
        while (n_drafting > 0) {
            ...
            for (llama_seq_id seq_id = 0; seq_id < (llama_seq_id) n_seq; ++seq_id) {
                ...
                // as well as target verification work. draft-mtp-adaptive owns the cap
                // (n_cap, set above); otherwise --spec-draft-adaptive sizes it from the
                // acceptance EMA, under the per-call context bound.
                const int32_t n_draft_eff = adaptive
                    ? n_cap[seq_id]
                    : std::min(n_cap[seq_id], adaptive_n_draft(seq_id, params.n_max, params.n_min));
